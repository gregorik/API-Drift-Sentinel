...
API Drift Sentinel monitors external APIs you depend on, not APIs you own.

It watches OpenAPI descriptions and developer docs, detects compatibility drift, extracts deadlines, maps changes to owners and internal services, runs proof checks, and creates routed follow-up actions such as Slack alerts, GitHub issues, Jira tickets, and audit timeline entries.

This is built for integration teams, platform teams, agencies, and AI/agent builders that need early warning before an upstream contract shift breaks production.
...
This repository includes the MIT license in [LICENSE](LICENSE).
